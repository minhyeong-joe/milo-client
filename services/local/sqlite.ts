...
async function initLocalDb(db: SQLite.SQLiteDatabase) {
  await db.execAsync(`
    ...
    CREATE TABLE IF NOT EXISTS immunization_payload_cache (
      user_id TEXT NOT NULL,
      baby_id TEXT NOT NULL,
      schedule_profile TEXT NOT NULL,
      schedule_items_json TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      PRIMARY KEY (user_id, baby_id)
    );

    CREATE TABLE IF NOT EXISTS immunization_record_cache (
      user_id TEXT NOT NULL,
      baby_id TEXT NOT NULL,
      id TEXT NOT NULL,
    ...
    CREATE INDEX IF NOT EXISTS immunization_record_cache_date_idx
    ON immunization_record_cache(user_id, baby_id, given_date DESC);

    CREATE TABLE IF NOT EXISTS immunization_mutation_queue (
      id TEXT PRIMARY KEY NOT NULL,
      user_id TEXT NOT NULL,
      baby_id TEXT NOT NULL,
      operation TEXT NOT NULL CHECK(operation IN ('create', 'update', 'delete', 'profile')),
      record_id TEXT,
      local_id TEXT,
      payload_json TEXT NOT NULL,
      status TEXT NOT NULL CHECK(status IN ('pending', 'failed')),
    ...
    CREATE INDEX IF NOT EXISTS immunization_mutation_queue_pending_idx
    ON immunization_mutation_queue(user_id, baby_id, status, created_at);
